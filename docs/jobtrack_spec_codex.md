...
### 4.5 Decomposing a leaf

A leaf may be genuinely decomposed into child jobs before work on it has begun as well as after
(ADR 0067) -- the operation shall be explicit and atomic in either case.

If the leaf already has `LeafWork` attached:
...
If the leaf is bare (no `LeafWork` ever attached), there is no existing work to carry over: no child
is created for it, and the operation simply creates the newly identified child jobs and converts the
original node into their branch parent. At least one newly identified child is required in this case,
since a decomposition producing zero children would leave the node in neither a leaf nor a branch
state. A node that already has children (already a branch) may never be decomposed, in either case.
...
Roles may be combined. `Cost viewer` is deliberately separate because visibility of employees does not imply visibility of their rates or costs. `Requester` is the exception to additive technical authority: while an account holds `Requester`, it is ineligible to own a job node, be named on new work, pick up work, or exercise structural/work-session authority even if it also holds a workflow role. Granting `Requester` to an account that currently owns a node or has an active session shall be rejected; technical work must be reassigned or finished first.
