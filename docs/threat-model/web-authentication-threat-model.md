...
| 13 | Database credential compromise | An actor with one application database login attempts to read Identity secret columns, exfiltrate/forge personal access tokens, escalate roles, or bypass audit append-only enforcement directly | Least-privileged runtime roles (security review remediation §2.6): `jobtrack_identity` verifies credentials and can only read role assignments; `jobtrack_domain` handles domain commands but cannot issue/authenticate/list PATs; `jobtrack_pat_management` performs the audited PAT lifecycle; `jobtrack_pat_authentication` can execute only token lookup/last-used and has no table grants. PAT table access remains behind source-controlled `SECURITY DEFINER` functions. **Documented residual**: `jobtrack_domain` still shares full `identity_user` secret-column access with `jobtrack_identity`, because credential-transition command ports update those columns in the same transaction as their audit row; `jobtrack_domain` also retains direct append-only audit insertion. | `TC-DB-ROLES-002`, `TC-DB-ROLES-004` |
