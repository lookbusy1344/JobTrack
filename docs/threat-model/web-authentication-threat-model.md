...
| 15 | Stolen-cookie credential minting/administration | A cookie stolen without the account password (XSS, network capture, shared-device residue) used to mint a PAT, bind an attacker-controlled second factor, provision an employee account, or perform administrator credential/role actions | `[RequiresRecentAuthentication]` (ADR 0057, §2.2) gates PAT issuance, self-service 2FA enable/disable, administrator employee creation/password reset/2FA reset/enable-disable/PAT-revoke-all, and role assignment behind `/Account/ConfirmAccess`, which re-verifies the current password through the lockout-tracked `CheckPasswordSignInAsync` path and places the whole password/TOTP attempt behind per-account-and-origin plus per-origin limiter budgets. Step-up also accepts one owned user-verified passkey (ADR 0071) with no additional TOTP | `TC-WEB-AUTHN-010` (new) |
