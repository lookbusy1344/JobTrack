...
|---|---|---|---|---|
| 1 | Credential stuffing | Scripted repeated login attempts against known/guessed username lists | Partitioned login/two-factor rate limiting keyed by remote address plus normalized username, with a separately bounded origin-backstop pool that survives primary-key rotation and evicts under storage pressure rather than denying all unseen callers; bounded lockout and generic failure responses | `TC-WEB-AUTHN-003` (new) |
| 2 | Account enumeration | Comparing login/reset response timing or content for existing vs. non-existing usernames | Generic failure messages that do not reveal account existence; unknown, disabled, locked, and wrong-password login outcomes each perform one current-policy password verification (spec §7.1) | `TC-WEB-AUTHN-004` (new) |
| 3 | Session theft | Cookie exfiltration (XSS, network capture, shared-device residue) reused after logout/disablement/password change | `Secure`/`HttpOnly`/`SameSite` cookies; security-stamp revocation on disablement, reset, password change, logout, and self-service 2FA changes; three distinct timeouts (ADR 0057, §2.3): an idle/renewal window (`SlidingExpiration`, unchanged), and — new — an absolute session ceiling from the original sign-in that `SlidingExpiration` cannot extend past, enforced in `OnValidatePrincipal` regardless of activity | `TC-WEB-AUTHN-005` (new) |
...
| 13 | Database credential compromise | An actor with one application database login attempts to read Identity secret columns, exfiltrate/forge personal access tokens, escalate roles, or bypass audit append-only enforcement directly | Least-privileged runtime roles: `jobtrack_domain` sees only non-secret account facts and cannot mutate `identity_user`/`identity_user_role`; `jobtrack_credential_administration` alone backs credential and role commands; a trigger rejects credential/role/authentication audit operation names from the domain role. Identity, PAT management/authentication and retained-history deletion remain independently credentialed. PAT and history mutations remain behind reviewed `SECURITY DEFINER` capabilities. **Residual:** a domain credential can still append and misattribute audit events for domain mutation names it genuinely serves; database roles do not cryptographically identify the HTTP actor. | `TC-DB-ROLES-002`, `TC-DB-ROLES-004`; `PostgreSqlRoleGrantsTests.The_domain_role_cannot_administer_credentials_roles_or_their_audit_trail` |
...
| 15 | Stolen-cookie credential minting/administration | A cookie stolen without the account password (XSS, network capture, shared-device residue) used to mint a PAT, bind an attacker-controlled second factor, provision an employee account, or perform administrator credential/role actions | `[RequiresRecentAuthentication]` (ADR 0057, §2.2) gates PAT issuance, self-service 2FA enable/disable, administrator employee creation/password reset/2FA reset/enable-disable/PAT-revoke-all, and role assignment behind `/Account/ConfirmAccess`. Password change, TOTP disablement, and confirmation use one current-credential boundary: request-and-account limiter budgets are consumed before verification, the account budget cannot be bypassed by changing source address, and wrong reusable passwords/TOTP codes share login's five-attempt account lockout ceiling. Password change records failure or replacement atomically; confirmation resets failures only after every required factor succeeds and audits TOTP failure/lockout without the code. Step-up also accepts one owned user-verified passkey (ADR 0071) with no additional TOTP | `TC-WEB-AUTHN-010` (new) |
| 16 | Passkey registration state tampering / cross-account binding | An altered or replayed creation ceremony state, or an attestation whose user handle differs from the signed-in employee, attempts to bind a credential to the wrong account | Microsoft's `IPasskeyHandler` owns challenge/origin/RP-ID/attestation validation over `SignInManager`'s own protected single-use state; the completion handler compares the returned user entity/handle against the signed-in employee before any row is stored; `credential_id` is globally unique so one credential cannot attach to two accounts (ADR 0071, plan §8.1) | `TC-WEB-AUTHN-011` (new) |
| 17 | RP ID / origin / Host-header confusion, subdomain takeover | A forwarded or spoofed Host header, or a takeover of a sibling subdomain, used to satisfy WebAuthn origin/RP-ID checks | Explicit configured `ServerDomain` (RP ID) and exact HTTPS origin allowlist — no suffix matches, wildcards, forwarded hosts, or HTTP origins; production startup fails when the feature is enabled without valid values matching the canonical host (ADR 0071 §6, plan §5) | `TC-WEB-AUTHN-012` (new) |
| 18 | Assertion replay / counter regression / synced-passkey zero counter | A captured assertion is replayed, or a metadata update lowers a non-zero signature counter under a race; synced passkeys legitimately report a zero counter | Framework assertion validation plus a store update that never lowers a non-zero `sign_count` (monotonic, race-tested); a zero counter from a synced authenticator is accepted without treating it as regression (ADR 0071 §6.1, plan §6.2) | `TC-DB-AUTHN-001` (new) |
| 19 | Credential / account enumeration through passkey surfaces | Options, error text, timing, audit, or the management UI reveal whether a username has passkeys or whether a credential exists | Username-less conditional and explicit options; generic failure for every malformed/expired/unknown/wrong-origin/replayed/disabled case; separate option/assertion limiter budgets scoped to the request's remote origin; no client-supplied credential ID used to select a message, actor, lockout target, or limiter partition; management UI shows only owned credentials to the owner (ADR 0071 §9, plan §8.2/§7.2) | `TC-WEB-AUTHN-013` (new) |
| 20 | Stolen-cookie passkey enrolment / removal | A stolen cookie used to add or remove a passkey without recent authentication | Add/remove/reset require ADR 0057 recent authentication; add/remove/reset rotate stamps, revoke PATs and other sessions, and audit (ADR 0071 §7, plan §8.1) | `TC-WEB-AUTHN-014` (new) |
| 21 | Administrator / CLI reset-passkeys abuse | The reset-passkeys admin/CLI path used outside its audited, least-privileged, recent-auth-gated intent | Administrator + recent-auth gate on the web path; narrow emergency capability for the CLI; one atomic command deletes all target credentials, rotates stamps, revokes sessions, and audits without printing credential data; reset-passkeys never touches password or TOTP (ADR 0071 §8, plan §8.4) | `TC-WEB-AUTHN-015` (new) |
| 22 | Multi-instance ceremony completion | The two ceremony requests land on different Cloud Run instances, breaking protected-state decryption | `SignInManager`'s ceremony cookie uses ADR 0066's shared Data Protection key ring, so the two requests may land on different hosts without affinity; a two-instance acceptance test pins this (ADR 0071, plan §11) | `TC-WEB-AUTHN-016` (new) |
| 23 | Credential data leakage | Credential ID, name, AAGUID, public key, attestation, client data, transports, or user handle reach logs, telemetry, audit, grants, exceptions, or exports | Stronger secret-handling rule (spec §7.1, ADR 0071): no passkey field in ordinary queries, logs, traces, audit payloads, exports, error detail, or reporting-role grants; audit records actor/target/correlation/outcome and a count only; reporting roles receive no table access | `TC-WEB-AUDIT-003` (new); `TC-DB-ROLES-005` (new) |
...
|---|---|---|---|---|
| 13 | Token theft | A PAT is exfiltrated from a compromised CLI host, shell history, log file, or committed config | Only a one-way hash of the token is stored server-side (unsalted SHA-256; the token's own 256 bits of entropy make a per-token salt redundant — ADR 0029); the plaintext is shown once at issuance and never logged; the owner or an administrator can revoke a specific token on suspicion without affecting others. **Documented residual** (ADR 0066 Stage 4): the issuance-to-display handoff is a short-lived, actor-bound, data-protected `JobTrack.PendingPat` cookie rather than the old in-process store's true single-consumption dictionary entry, so a client that captures and replays the raw cookie value within its ~2-minute window can decrypt it a second time — deletion is a Set-Cookie instruction a compliant browser honors, not server-enforced one-time use. That replay can only ever reveal a token already delivered once to that same authenticated actor (the protector purpose is bound to Identity's own row id); it grants no capability the actor's own browser session did not already have, and it self-expires quickly. | `TC-WEB-TOKEN-001` (new) |
