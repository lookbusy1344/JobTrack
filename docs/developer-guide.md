...
Generates every credential and certificate fresh into a mode-700 temporary directory, brings up
`docker/compose.multi-instance.yml`, waits for both hosts' `/health/ready`, then drives a black-box
scenario across host A and host B (alternating which one receives each request): cross-host
authentication, antiforgery, remembered-filter recall, personal-access-token delivery, both the login
and API rate limiters, and a concurrent domain-write race. On failure it dumps the stack's container
logs with every generated secret redacted, then always tears down its own containers, network, and
volumes via a trap -- nothing it creates is left behind, committed, or printed. Separate from the
Homebrew PostgreSQL developer default above; it never touches `jobtrack_dev` or `jobtrack-web-dev.db`.
