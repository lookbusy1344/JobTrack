...
**Every filter the dashboard offers is remembered per session, except the subtree scope.** Owner was
already; the unassigned pool, search text, the in-progress narrowing and the new blocked-job
exclusion now are too, through `FilterMemory`'s flag and text forms. A request that names one of
those sets it and remembers it; a request that names none — the header nav link, a PRG redirect that
lost one — restores the last choice instead of resetting the view.

Two things are deliberately not remembered. `Offset` is a position within a result, not a filter.
The subtree scope pair (`SubtreeRootId` with its "show the whole tree" companion) is a *place*: a URL
naming no node always scopes to the actor's home node, falling back to the whole tree for an actor
with none, so the header link is a reliable way home rather than a replay of wherever the last visit
ended up. The scope still survives every within-page round trip, because the filter form, the PRG
redirects and the paging links all carry it explicitly.

**The dashboard is always scoped to exactly one node, and the whole tree is the root being that
node.** `ShowWholeTree` is an inbound shorthand, not a second kind of scope: it resolves to the
root's own id while the page loads, so the scope line reads "Scoped to *root*" like any other node
and the toolbar's Browse button opens it. Nothing records where the viewer came from — a scope is a
place, not a journey. The subtree filter is dropped when the scope node is the root, since "every
leaf under the root" and "every leaf" are the same set and the query port should not plan a
containment test that excludes nothing.

**The scope line names its alternatives as places.** Each alternative reads "Show *place*" with only
the place a link, in the small `jt-value-aside` size, so the line's reading weight stays on the node
it is scoped to. There are two: "whole tree", drawn unless the scope already is the root, and "home
node", drawn whenever the actor has one and the dashboard is not already showing exactly that
subtree — the same place a node-less URL resolves to, reachable in one click from wherever the viewer
wandered.
