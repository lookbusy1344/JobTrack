...
### 3.5 Decomposing a leaf

A leaf may be genuinely decomposed before work on it has begun as well as after (ADR 0067), one
atomic op either way. If it already has `LeafWork` attached: (1) create a child for the work already
done; (2) move the existing `LeafWork` and all its sessions to that child; (3) create the newly
identified child jobs; (4) convert the original node into their branch parent. Session identifiers,
users, times, audit history, and rate semantics are preserved. This is never used for mere
pause/resume.

If the leaf is bare (no `LeafWork` ever attached), there is nothing to carry over: step (1)/(2) are
skipped, and the operation just creates the newly identified child jobs and converts the node into
their branch parent. At least one new child is required in this case -- zero would leave the node in
neither a leaf nor a branch state. A node that already has children may never be decomposed.
