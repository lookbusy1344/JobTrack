...
| [`2026-07-23-architecture-boundary-remediation-plan.md`](2026-07-23-architecture-boundary-remediation-plan.md) | Implemented | Fresh-eyes architecture review: move split work-ending and password-change workflows behind atomic library commands, tighten the public SPI and EF boundary, and add one-intent/one-mutation guardrails. |
| [`2026-07-24-code-review-scalability-remediation-plan.md`](2026-07-24-code-review-scalability-remediation-plan.md) | Implemented | Full-project review: read-path scaling ceiling (full-table hierarchy loads, cost read scope), plan-status drift, single-instance store inventory, and maintainability hotspots. |
| [`2026-07-25-scalability-follow-up-plan.md`](2026-07-25-scalability-follow-up-plan.md) | Implemented | Remaining request-scoping, search, cost-query, and benchmark work following the initial scalability remediation commits. |
| [`2026-07-26-multi-instance-web-deployment-plan.md`](2026-07-26-multi-instance-web-deployment-plan.md) | Proposed (blocked on a superseding ADR for ADR 0014) | What must change before `JobTrack.Web` can run as 2+ instances against one shared PostgreSQL: shared data-protection key ring, the four in-process stores, health endpoints, and rolling-deploy schema discipline. |
| [`2026-07-26-framework-design-guidelines-compliance-plan.md`](2026-07-26-framework-design-guidelines-compliance-plan.md) | Implemented (6 of 8; §2.5 and §2.6 withdrawn with evidence) | Library-surface audit against `../Framework_Design_Guidelines_Essentials.md`: unintended `Persistence.Shared` public surface, missing `CLSCompliant`, incomplete `Money`/`HourlyRate` value-type contract, `AllocatedShare` default state, corrupted entry-point docs, and `static readonly` constant tables. |
| [`2026-07-28-fresh-eyes-code-review-remediation-plan.md`](2026-07-28-fresh-eyes-code-review-remediation-plan.md) | Implemented (self-referential fast-runner harness withdrawn by owner) | Fresh-eyes review: achievement-reopen serialization, blocked-query fan-out, deterministic performance gates, fast-suite budget enforcement, principal-bound filter state, and immutable test guardrails. |
| [`2026-08-01-security-audit-remediation-plan.md`](2026-08-01-security-audit-remediation-plan.md) | Implemented (two explicit §2.6 residual risks; live trusted-CA and process-inspection tests deferred) | Fresh security audit: password and session assurance, library read admission, PAT issuance authority, PostgreSQL role/transport boundaries, CLI secret handling, and login-limiter availability. |
