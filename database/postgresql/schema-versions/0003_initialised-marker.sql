-- Schema version 0003 (PostgreSQL): the singleton initialised_marker table
...
CREATE FUNCTION initialised_marker_immutable() RETURNS trigger AS
$$
BEGIN
    RAISE EXCEPTION 'initialised_marker is append-only and cannot be updated or deleted (ADR 0015)';
END;
$$ LANGUAGE plpgsql;
...
CREATE TRIGGER initialised_marker_no_update
    ...
    FOR EACH ROW
EXECUTE FUNCTION initialised_marker_immutable();
