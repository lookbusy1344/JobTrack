	...
	: WorkSessionCommandPortContractTestsBase(new PostgreSqlDatabaseFixture())
{
	...
	///     starting its dependent still serialize around the dependent's readiness decision (here on the
	///     prerequisite's own advisory lock), but the two are no longer symmetric. The reopen always wins
	///     its own outcome -- it is never rejected because a dependent is live -- while the dependent's
	///     start survives only if it committed while the prerequisite was still Success. Neither may
	///     commit from a snapshot the other has already invalidated: a start that loses leaves no session.
	/// </summary>
	[Fact]
