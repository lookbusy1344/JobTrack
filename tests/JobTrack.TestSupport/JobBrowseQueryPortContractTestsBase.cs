...
public abstract class JobBrowseQueryPortContractTestsBase : IAsyncLifetime
{
	...
	/// <summary>
	///     Asking for the root of a tree that has none is the same kind of miss as asking for an id that
	///     does not resolve, and reaches the same callers (a page loading its default scope on a system
	///     whose tree has not been bootstrapped yet) — so it is the same exception, not the raw
	///     sequence-empty error that would surface as a server fault.
	/// </summary>
	[Fact]
	public async Task GetNodeAsync_with_null_throws_when_the_tree_has_no_root()
	{
		await DeploySchemaAsync();
		var port = CreateBrowsePort(database.ConnectionString);

		var act = () => port.GetNodeAsync(null);
	...
	[Fact]
	public async Task GetSubtreeAsync_reports_a_displayed_branch_rollup_from_beyond_the_render_depth()
	{
		var (rootId, branchId, tree) = await SeedTreeAsync();
		await SucceedLeafAsync(tree.CabinetsLeafId, tree.JobManagerId);
		await SucceedLeafAsync(tree.PlumbingLeafId, tree.JobManagerId);
	...
	public async Task GetSubtreeAsync_returns_leaf_achievement_and_root_rollup_from_one_snapshot()
	{
		...
		var interceptor = new BlockingReaderCommandInterceptor(sql =>
			sql.Contains("AchievementId", StringComparison.Ordinal));
	...
	/// </summary>
	/// <summary>Brings the schema up without seeding anything into it — an empty tree, as a fresh deployment has.</summary>
