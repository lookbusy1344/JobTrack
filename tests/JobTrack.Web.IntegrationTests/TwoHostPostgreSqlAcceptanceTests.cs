...
public sealed partial class TwoHostPostgreSqlAcceptanceTests : IAsyncLifetime, IDisposable
{
	...
	private static readonly FrozenDictionary<string, string?> PostgreSqlDataProtectionStoreSetting =
		new Dictionary<string, string?> {
			...
			["Authentication:Passkeys:Enabled"] = "true",
			["Authentication:Passkeys:ServerDomain"] = "localhost",
			["Authentication:Passkeys:Origins:0"] = "http://localhost",
		}.ToFrozenDictionary();
	...
	public async Task Passkey_assertion_state_generated_by_host_A_is_accepted_by_host_B()
	{
		...
		optionsRequest.Headers.Add(AntiforgeryHeaderName, token);
		var optionsResponse = await clientA.SendAsync(optionsRequest);
		var ceremonyCookies = string.Join("; ", new[] {
			antiforgeryCookie,
		}.Concat(SetCookiePairs(optionsResponse)));

		using var assertionRequest = new HttpRequestMessage(HttpMethod.Post, "/Account/Login?handler=PasskeySignIn");
		assertionRequest.Headers.Add("Cookie", ceremonyCookies);
		assertionRequest.Headers.Add(AntiforgeryHeaderName, token);
		assertionRequest.Content = new StringContent(
		...
		var response = await clientB.SendAsync(assertionRequest);
		var body = await response.Content.ReadAsStringAsync();

		body.Should().Contain("That passkey could not be used to sign in");
		body.Should().NotContain("expired", "host B must decrypt the ceremony state generated by host A");
	}
