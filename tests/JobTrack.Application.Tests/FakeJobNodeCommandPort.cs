...
internal sealed class FakeJobNodeCommandPort : IJobNodeCommandPort, IReadinessQueryPort, IJobBrowseQueryPort, IAwaitingProgressQueryPort
{
		...
		DecomposeWorkedLeafRequest request, CancellationToken cancellationToken = default)
	{
		...
		if (_nodes.Values.Any(n => n.ParentId == existing.Id)) {
			throw new InvariantViolationException(
				"job-node-has-children-cannot-decompose", "A node with children cannot be decomposed.");
		}

		var hasLeafWork = _leafWork.TryGetValue(request.LeafNodeId, out var leafWork);
		if (!hasLeafWork && request.NewChildren.Count == 0) {
			throw new InvariantViolationException(
				"job-node-decompose-requires-a-child",
				"A leaf with no recorded work needs at least one new child to decompose into.");
		}

		JobNodeResult? existingWorkChild = null;
		if (hasLeafWork) {
			existingWorkChild = WithStructuralFacts(new() {
				Id = new(_nextId++),
