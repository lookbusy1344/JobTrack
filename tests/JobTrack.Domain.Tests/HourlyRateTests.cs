...
public sealed class HourlyRateTests
{
	...
	[Fact]
	public void Two_instances_with_the_same_rate_are_equal() => new HourlyRate(9.5m).Should().Be(new(9.5m));
	...
	[Fact]
	public void A_smaller_rate_compares_below_a_larger_one() => new HourlyRate(9m).CompareTo(new(11m)).Should().BeNegative();
