...
public sealed class PostgreSqlRoleGrantsTests : IAsyncLifetime
{
	...
	public async Task Security_definer_authority_matches_the_reviewed_capability_matrix()
	{
		...
		actual.Should().Equal(
			...
			"jobtrack_identity:rate_limit_try_consume(p_purpose text, p_partition_digest bytea, p_backstop_digest bytea, p_now timestamp with time zone, p_window_seconds integer, p_permit_limit integer, p_backstop_permit_limit integer, OUT out_allowed boolean, OUT out_rows_pruned integer, OUT out_rows_evicted integer)",
			"jobtrack_identity:rate_limit_try_consume(p_purpose text, p_partition_digest bytea, p_backstop_digest bytea, p_now timestamp with time zone, p_window_seconds integer, p_permit_limit integer, p_backstop_permit_limit integer, p_max_partition_count integer, OUT out_allowed boolean, OUT out_rows_pruned integer, OUT out_rows_evicted integer)",
