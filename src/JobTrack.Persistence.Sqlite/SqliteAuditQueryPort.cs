...
/// <summary>
///     SQLite implementation of <see cref="IAuditQueryPort" /> (impl plan §7.3/§7.4 slice 11: query
///     audit history using sensitive-field projections). One <see cref="SqliteJobTrackDbContext" /> per
///     call, read-only throughout, matching every prior read-only port's shape. Materializes every
///     matching raw event unconditionally -- <see cref="AuditQueries" /> applies
///     <see
///         cref="Domain.Authorization.AuditAccessPolicy" />
///     and the per-event sensitive-field projection
///     itself.
/// </summary>
internal sealed class SqliteAuditQueryPort : IAuditQueryPort
{
	private readonly string connectionString;

	/// <summary>Creates the port over the given SQLite connection string.</summary>
	public SqliteAuditQueryPort(string connectionString) => this.connectionString = connectionString;
...
/// <summary>
///     The audit-search assembly logic behind <see cref="SqliteAuditQueryPort" />, mirrored
///     (necessarily duplicated, not literally shared) by PostgreSQL's own <c>AuditQueryAssembly</c> --
