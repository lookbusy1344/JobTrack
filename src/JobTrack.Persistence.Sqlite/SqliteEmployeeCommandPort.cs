...
/// <summary>
///     SQLite implementation of <see cref="IEmployeeCommandPort" /> (plan §8.3: employee role
...
///     that serializes concurrent writes through SQLite's single-writer model (matches
///     <see cref="SqliteRateCommandPort" />'s established use of the same technique). Role
...
internal sealed class SqliteEmployeeCommandPort : IEmployeeCommandPort
{
	...
	private static async Task RotateSecurityStampAsync(
		SqliteJobTrackDbContext context, long identityUserId, CancellationToken cancellationToken)
	{
	...
	private static async Task<(long TargetIdentityUserId, EquatableArray<EmployeeRole> Roles)> LoadTargetAsync(
		SqliteJobTrackDbContext context, AppUserId targetUserId, CancellationToken cancellationToken)
	{
