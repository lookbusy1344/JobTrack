...
///     schema version 0007's immediate triggers remain the authority under races and direct-write
///     bypass. Duplicated across the two providers (not across the callers within one), matching
///     this codebase's established convention (see <c>LeafReadiness</c>).
/// </summary>
internal static class LeafSessionClosure
{
	...
	public static async Task<bool> IsClosedAsync(SqliteJobTrackDbContext context, JobNodeId leafId, CancellationToken cancellationToken)
	{
	...
	public static Task<bool> HasActiveSessionAsync(SqliteJobTrackDbContext context, JobNodeId leafId, CancellationToken cancellationToken) =>
