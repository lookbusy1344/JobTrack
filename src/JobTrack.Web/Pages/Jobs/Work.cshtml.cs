	...
	ILogger<WorkModel> logger) : PageModel
{
	...
	///     unavailable — the same optional-field treatment as the leaf's own Cost record-card field on
	///     Browse (ADR 0039 decision 4/ADR 0040/ADR 0042): an unauthorized viewer or a session with no
	...
	private AppUserId? ResolveWorkerFilter(JobNodeId leafId)
	{
		...
		var store = new CookieFilterMemoryStore(HttpContext, dataProtectionProvider);
		var resolved = FilterMemory.Resolve(store, key, Request.Query.ContainsKey(nameof(WorkedByUserId)), WorkedByUserId, null);
