...
///     <see cref="PostgreSqlJobNodeCommandPort.AttachLeafWorkAsync" /> and
///     <see cref="PostgreSqlWorkSessionCommandPort.StartWorkAsync" />'s attach-if-missing step, so the
///     two callers cannot drift on what makes a node eligible to hold <c>LeafWork</c>. The caller is
...
internal static class LeafWorkAttachSupport
{
	...
	public static async Task<LeafWorkEntity> CreateAsync(
		PostgreSqlJobTrackDbContext context, JobNodeEntity node, Instant now, CommandContext commandContext,
