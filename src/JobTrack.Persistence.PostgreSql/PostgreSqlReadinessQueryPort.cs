...
internal sealed class PostgreSqlReadinessQueryPort(NpgsqlDataSource dataSource) : IReadinessQueryPort
{
		...
		JobNodeId nodeId, CancellationToken cancellationToken = default)
	{
		...
		return await LoadAsync(context, nodeId, cancellationToken).ConfigureAwait(false);
	}
