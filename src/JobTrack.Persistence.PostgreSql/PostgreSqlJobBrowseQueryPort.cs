...
/// <summary>
///     PostgreSQL implementation of <see cref="IJobBrowseQueryPort" /> (plan §8.5 slice 2). One
///     <see cref="PostgreSqlJobTrackDbContext" /> per call, read-only throughout. Reuses
...
/// </summary>
internal sealed class PostgreSqlJobBrowseQueryPort(NpgsqlDataSource dataSource) : IJobBrowseQueryPort
{
	...
	internal PostgreSqlJobBrowseQueryPort(NpgsqlDataSource dataSource, IReadOnlyList<IInterceptor> interceptors)
		: this(dataSource) =>
...
#pragma warning restore CA1304, CA1311, CA1862
	...
	{
		var achieved = await context.Database.SqlQuery<bool>(
				$"""
				 SELECT node_succeeded({rootId.Value}) AS "Value"
				 """)
