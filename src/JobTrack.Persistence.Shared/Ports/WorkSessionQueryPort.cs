...
/// <summary>
///     The provider-neutral body of <see cref="IWorkSessionQueryPort" /> (plan §8.5 slice 4). One
///     <see cref="DbContext" /> per call, read-only throughout.
/// </summary>
internal sealed class WorkSessionQueryPort(IWorkSessionQueryProviderOperations provider, IClock clock) : IWorkSessionQueryPort
{
	/// <inheritdoc />
		...
		AppUserId actorId, EquatableArray<JobNodeId> leafWorkIds, CancellationToken cancellationToken = default)
	{
		...
		if (leafWorkIds.Count == 0) {
			return new() {
				ActorRoles = actorRoles,
				ControlledLeafWorkIds = [],
			};
		}

		var controlledIds = await provider.GetControlledLeafIdsAsync(
			context, actorId.Value, [.. leafWorkIds.Select(id => id.Value)], cancellationToken).ConfigureAwait(false);

		return new() {
			ActorRoles = actorRoles,
			ControlledLeafWorkIds = [.. controlledIds.Select(id => new JobNodeId(id))],
		};
	}
