...
/// <summary>
///     The provider-neutral body of <see cref="IEmployeeCommandPort" /> (plan §8.3: employee role
///     assignment, account enable/disable, password reset). One context/connection/transaction per
///     call over <see cref="IProviderWriteOperations" />, reloading the actor's current roles and
///     applying <see cref="EmployeeAccessPolicy" /> itself before writing. Role
...
///     for nothing.
/// </summary>
internal sealed class EmployeeCommandPort(IProviderWriteOperations provider, IClock clock) : IEmployeeCommandPort
{
	/// <inheritdoc />
	...
	private static async Task AddDefaultScheduleAsync(
		DbContext context, AppUserId userId, string ianaTimeZone, Instant now, CancellationToken cancellationToken)
	{
