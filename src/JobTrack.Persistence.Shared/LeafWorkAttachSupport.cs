...
///     the provider job-node command port's <c>AttachLeafWorkAsync</c> and
///     <see cref="Ports.WorkSessionCommandPort.StartWorkAsync" />'s attach-if-missing step, so the
...
internal static class LeafWorkAttachSupport
{
	...
	public static async Task<LeafWorkEntity> CreateAsync(
		DbContext context, JobNodeEntity node, Instant now, CommandContext commandContext,
