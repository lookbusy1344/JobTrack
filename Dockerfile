...
#
# Build context MUST be the monorepo root (this file's parent's parent), NOT the
# JobTrack/ folder — .editorconfig lives one level above JobTrack/ (JobTrack/CLAUDE.md: "the git
# root is the parent monorepo directory") and the analyzer build below is TreatWarningsAsErrors, so
