...
for required in \
	...
	JOBTRACK_USER1_USERNAME JOBTRACK_USER1_ROLES \
	JOBTRACK_USER2_USERNAME JOBTRACK_USER2_ROLES; do
...
# Multi-instance connection budget (ADR 0066 §11). The deploy script computes how many connections
# each host's four pools may hold, from the max_connections its chosen Cloud SQL tier is documented to
# provide. That figure is a documentation lookup made on the operator's machine, which cannot reach
# this private instance to check it -- this job is the only component that can. Ask the server what it
# actually allows and refuse to provision against a smaller one, so a tier that silently failed to
# apply (or a documented default that changed) surfaces here rather than as pool exhaustion under load.
# Optional: unset means single-instance, where the budget does not apply.
if [[ -n ${JOBTRACK_EXPECTED_MAX_CONNECTIONS:-} ]]; then
	echo "==> verifying the database connection ceiling"
	actual_max_connections=$(psql --no-psqlrc --quiet --tuples-only --no-align \
		--set=ON_ERROR_STOP=1 --command 'SHOW max_connections')
	if ((actual_max_connections < JOBTRACK_EXPECTED_MAX_CONNECTIONS)); then
		echo "ERROR: this instance allows $actual_max_connections connections; the deployment's pool budget assumes at least $JOBTRACK_EXPECTED_MAX_CONNECTIONS." >&2
		echo "Raise the Cloud SQL tier, or lower max_instances/the per-pool maxima in scripts/deploy-cloudrun-postgresql.sh." >&2
		exit 1
	fi

	echo "    $actual_max_connections available, at least $JOBTRACK_EXPECTED_MAX_CONNECTIONS required"
fi
...
# Data-protection keys must be encrypted at rest. This is checked rather than assumed because the
# opposite went unnoticed for five days: the service ran before its certificate secret existed, so the
# key it generated then was persisted in plaintext, and adding the certificate afterwards encrypted
# only *subsequent* keys -- a key's stored form is fixed when it is created. Nothing surfaced it; it
# was found by chance while migrating the ring. An unencrypted key means anyone who can read the
# store holds every session cookie and TOTP secret it protects, so this fails the deployment.
#
# Runs after the schema deploy, so the table exists. A fresh database has no keys yet and passes
# trivially -- the application creates the first one on demand, by which time the certificate is
# mounted and startup has already refused to run without it. Reports counts only, never key material.
echo "==> verifying data-protection keys are encrypted at rest"
unencrypted_key_count=$(psql --no-psqlrc --quiet --tuples-only --no-align --set=ON_ERROR_STOP=1 \
	--command "SELECT count(*) FROM data_protection_key WHERE xml IS NOT NULL AND xml NOT LIKE '%EncryptedData%';")
total_key_count=$(psql --no-psqlrc --quiet --tuples-only --no-align --set=ON_ERROR_STOP=1 \
	--command "SELECT count(*) FROM data_protection_key;")
if ((unencrypted_key_count > 0)); then
	echo "ERROR: $unencrypted_key_count of $total_key_count data-protection keys are stored unencrypted." >&2
	echo "A key is encrypted only if the certificate was configured when it was created; adding one later does not re-encrypt it." >&2
	echo "Confirm DataProtection:CertificatePath/CertificatePasswordPath are mounted, then retire the affected keys." >&2
	exit 1
fi

echo "    $total_key_count key(s), all encrypted"
...
if [[ $initialised == 0 ]]; then
	require_env JOBTRACK_ADMIN_PASSWORD
...
create_employee() {
	...
	if [[ -z $password ]]; then
		echo "ERROR: no enrolment password is available for missing employee '$username'." >&2
		echo "Restore an enabled version of its account-password secret, then rerun the deployment." >&2
		exit 2
	fi
