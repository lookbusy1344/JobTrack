...
# Angle brackets, not braces: a '}' inside a ${var:?message} closes the expansion at that character,
# so `${1:?Usage: $0 {password|two-factor} <username>}` silently evaluated to $1 with the literal text
# ' <username>}' appended -- every invocation reached the mode check as "two-factor <username>}" and
# was rejected as an unknown mode. This recovery path had therefore never run successfully.
